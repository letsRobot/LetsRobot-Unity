...
int32_t setpointA = 3576  / cRate;   // Desired Motor Velocity (Ticks/Second)
int32_t setpointB = 3576 / cRate;   // Desired Motor Velocity (Ticks/Second)
int32_t setpointAdefault = 3576 / cRate;
int32_t setpointBdefault = 3576 / cRate;
...
uint8_t KP = 3;                // Propoprtional Gain Constant
...
uint16_t deg_360 = 5800;
bool motion_done = true;
bool pos_control = false;

int16_t X_val = 0;
int16_t Y_val = 0;
bool analog_good = false;
uint32_t analog_timer = 0;
uint8_t analog_speed = 3;
...
void stop(void) {
  ...
  if (open_grip == false)
  {
  analogWrite(GRIP_PWM,grip_strength);
  }
  else
  {
...
#define EYE_BLINK_COLOR "30 10 0"
...
//  add_command("r",        cmd_right);
  ...
  add_command("setds",    cmd_yaxis);
  add_command("setts",    cmd_xaxis);
  ...
  add_command("vel", cmd_vel_control);
  add_command("sgrip", cmd_grip_strength);
  add_command("stick", cmd_analog_control);
  add_command("analogtoggle",cmd_toggle);
  ...
  */
  ...
  Motor_Setup(GRIP_PWM,GRIP_INA,GRIP_INB);
